...
fun Model.enableSeries(
    interval: Interval,
    historyLen: Int = 100,
    interpolated: Boolean = true
): List<TimeSeries<Ohlc>> {
...
fun Model.closePositionByTimeout(
    afterTime: LocalTime? = null,
    periods: Int,
    interval: Interval
) {
...
fun Model.price(idx: Int) : Double{
    return context.mdDistributor.price(idx).close
}