...
class ColDefDao<T>(val path: Path, val colDefs : Array<ColDef<T, out Any>>, val tableName : String, val pk : List<String> = emptyList()) {
    ...
    init {
        JdbcTemplate(ds).execute(SqlUtils.makeCreateSqlStmtFromHeader(tableName, getHeader(colDefs), pk))
    }
