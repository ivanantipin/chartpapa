...
class EodHistSource : HistoricalSource {
    ...
    override fun symbols(): List<InstrId> {
        val cachedSymbols = CacheService.getCached("eodhist_symbols", {
            ...
            val url =
                "https://eodhistoricaldata.com/api/exchange-symbol-list/US?api_token=5e81907493e611.25433232&fmt=json"
        ...
        return String(cachedSymbols).readJson().filter {
            ...
            InstrId(
                id = it["Code"].textValue(),
                code = it["Code"].textValue(),
                name = it["Name"].textValue(),
                market = it["Exchange"].textValue(),
                source = SourceName.EODHIST.name
            )
        }
    }
    ...
    override fun load(instrId: InstrId, dateTime: LocalDateTime, interval: Interval): Sequence<Ohlc> {
        ...
        val finam = mdStorageImpl.read(
            instrId.copy(source = "FINAM", market = FinamDownloader.BATS_MARKET),
            Interval.Min10,
            dateTime
        )
        ...
        if (dateTime.isBefore(LocalDateTime.now().minusDays(100)) && !mdExists(instrId)) {
        ...
        return sequence {
            ...
            while (true) {
                ...
                val url =
                    "https://eodhistoricaldata.com/api/intraday/${instrId.code}.US?fmt=json&api_token=5e81907493e611.25433232&interval=1m&from=${dt}&to=${dtto}"
