...
class StreamTradeCaseWriter(val path : Path, val factors : Iterable<String>) : (Trade)->Unit{
    ...
    override fun invoke(trade : Trade): Unit {
        val gen=generatorMap.computeIfAbsent(trade.security(),{StreamTradeCaseGenerator()})
        val serializer = TradeSerializer()
        for(c in gen(trade)){
            stream.write((serializer.serialize(c) + factors.map {c.first.tradeStat.factors[it]})
